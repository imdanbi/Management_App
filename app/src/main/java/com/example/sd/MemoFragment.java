...
public class MemoFragment extends Fragment {
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == ADD_NODE_REQUEST && resultCode == RESULT_OK){
            ...
            Toast.makeText(getActivity(), "노트가 저장되었습니다!", Toast.LENGTH_SHORT).show();
        }
        else if(requestCode == EDIT_NODE_REQUEST && resultCode == RESULT_OK){
            int id = data.getIntExtra(AddNoteActivity.EXTRA_ID, -1);
            if(id == -1){
                Toast.makeText(getActivity(), "수정 할 수 없습니다.", Toast.LENGTH_SHORT).show();
                return;
            }
            String title = data.getStringExtra(AddNoteActivity.EXTRA_TITLE);
            String description = data.getStringExtra(AddNoteActivity.EXTRA_DESCRIPTION);
            ...
            Toast.makeText(getActivity(), "메모가 저장되지 않았습니다!", Toast.LENGTH_SHORT).show();
        }
    }
